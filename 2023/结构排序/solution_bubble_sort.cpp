...
int main(){
	...
	//sort(s,s+count_stu-1,cmp);
	for(int i=0;i<count_stu;i++){
		for(int j=0;j<count_stu-1;j++){
			if(s[j].cno[2]>s[j+1].cno[2]){
				stu temps;
				strcpy(temps.cno,s[j].cno);
				strcpy(temps.sno,s[j].sno);
				temps.avg=s[j].avg;
				strcpy(s[j].cno,s[j+1].cno);
				strcpy(s[j].sno,s[j+1].sno);
				s[j].avg=s[j+1].avg;
				strcpy(s[j+1].cno,temps.cno);
				strcpy(s[j+1].sno,temps.sno);
				s[j+1].avg=temps.avg;
			}else if(s[j].cno[2]==s[j+1].cno[2]){
